...
export function validateGeneratedCode(html: string): ValidationResult {
  ...
  // 危险模式列表
  // 注意：window.parent 和 parent.postMessage 是允许的，用于 viewer 页面通信
  const forbiddenPatterns = [
    ...
    // { pattern: /window\.parent/gi, message: '禁止访问父窗口' },  // 允许，用于 viewer 通信
    // { pattern: /window\.top/gi, message: '禁止访问顶层窗口' },   // 允许，用于检测 iframe
...
export function autoFixCode(html: string): string {
  ...
  // 不再注释 window.parent，因为需要用于 viewer 通信
